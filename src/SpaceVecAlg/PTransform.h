 ...
 */
template <typename T> Matrix3<T> RotX(T theta);
 ...
 * @param theta rotation in radian.
 */
template <typename T> Matrix3<T> RotY(T theta);
 ...
 * @param theta rotation in radian.
 */
template <typename T> Matrix3<T> RotZ(T theta);
 ...
 */
template <typename T> Vector3<T> rotationVelocity(const Matrix3<T> &E_a_b);
...
public:
  ...
  template <typename T2> PTransform<T2> cast() const {
...
template <typename T> inline Matrix3<T> RotX(T theta) {
  T s = std::sin(theta), c = std::cos(theta);
  return (Matrix3<T>() << 1., 0., 0., 0., c, s, 0., -s, c).finished();
}

template <typename T> inline Matrix3<T> RotY(T theta) {
  T s = std::sin(theta), c = std::cos(theta);
  return (Matrix3<T>() << c, 0., -s, 0., 1., 0., s, 0., c).finished();
}

template <typename T> inline Matrix3<T> RotZ(T theta) {
  T s = std::sin(theta), c = std::cos(theta);
...
PTransform<T> interpolate(const PTransform<T> &from, const PTransform<T> &to,
  ...
  PTransform<T> result(qfrom.slerp(t, qto),
                       (from.translation() * (1. - t) + to.translation() * t));
  return result;
}
